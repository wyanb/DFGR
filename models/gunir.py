...
class GUniR(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.transformer_guide = Transformer(
            hidden_dim, feature_size, neighbor_mask, **kwargs
        )
        ...
        self.nf_fast_flow2 = nf_fast_flow(
            # [in_channels, int(input_size / scale), int(input_size / scale)],
            [272, int(14), int(14)],
            conv3x3_only=False,
            hidden_ratio=1.0,
    ...
    def forward(self, input):
        ...
        feature_resize_level1 = feature_resize_level_list[0]
                
        feature_tokens = rearrange(
            feature_align, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        feature_rec_list = []
        feature_list = input["feature_list"]  # B x C X H x W 3 
        feature_list = feature_list[0]  # B x C X H x W 3 
        feature_align_level = feature_resize_level_list[0]
        ...
        feature_rec_1 = rearrange(
            feature_rec_tokens, "(h w) b c -> b c h w", h=self.feature_size[0]
        )  # B x C X H x W
        feature_rec_1, log_jac_dets = self.nf_fast_flow2(feature_rec_1)
        feature_rec_list.append(feature_rec_1)

        fea_lev1_rec = feature_resize_level1 + feature_rec_1
        fea_lev1_rec, log_jac_dets = self.nf_fast_flow(fea_lev1_rec)
        feature_tokens_guide = rearrange(
            #feature_align, "b c h w -> (h w) b c"
            fea_lev1_rec, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        if self.training and self.feature_jitter:
            feature_tokens_guide = self.add_jitter(
                feature_tokens_guide, self.feature_jitter.scale, self.feature_jitter.prob
            )
        feature_tokens_guide = self.input_proj(feature_tokens_guide)  # (H x W) x B x C
        pos_embed_guide = self.pos_embed(feature_tokens_guide)  # (H x W) x C
        output_decoder_guide, _ = self.transformer_guide(
            feature_tokens_guide, pos_embed_guide
        )  # (H x W) x B x C
        feature_rec_tokens_guide = self.output_proj(output_decoder_guide)  # (H x W) x B x C
        feature_rec_guide = rearrange(
            feature_rec_tokens_guide, "(h w) b c -> b c h w", h=self.feature_size[0]
        )  # B x C X H x W
        feature_rec_guide, log_jac_dets = self.nf_fast_flow2(feature_rec_guide)
        feature_rec_list.append(feature_rec_guide)
        
        fea_trans_input3 = feature_align + feature_rec_guide + feature_resize_level1
        fea_trans_input3, log_jac_dets = self.nf_fast_flow(fea_trans_input3)
        feature_trans_input3 = rearrange(
            fea_trans_input3, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        if self.training and self.feature_jitter:
            feature_trans_input3 = self.add_jitter(
                feature_trans_input3, self.feature_jitter.scale, self.feature_jitter.prob
            )
        feature_trans_input3 = self.input_proj(feature_trans_input3)  # (H x W) x B x C
        pos_embed_3 = self.pos_embed(feature_trans_input3)  # (H x W) x C
        feature_rec_out3, _ = self.transformer_3(
            feature_trans_input3, pos_embed_3
        )  # (H x W) x B x C
        feature_rec_out3 = self.output_proj(feature_rec_out3)  # (H x W) x B x C
        feature_rec_3 = rearrange(
            feature_rec_out3, "(h w) b c -> b c h w", h=self.feature_size[0]
        )  # B x C X H x W
        feature_rec_3, log_jac_dets = self.nf_fast_flow2(feature_rec_3)
        feature_rec_list.append(feature_rec_3)
        
        fea_trans_input4 = feature_rec_3 + feature_resize_level1
        fea_trans_input4, log_jac_dets = self.nf_fast_flow(fea_trans_input4)
        feature_trans_input4 = rearrange(
            fea_trans_input4, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        if self.training and self.feature_jitter:
            feature_trans_input4 = self.add_jitter(
                feature_trans_input4, self.feature_jitter.scale, self.feature_jitter.prob
            )
        feature_trans_input4 = self.input_proj(feature_trans_input4)  # (H x W) x B x C
        pos_embed_4 = self.pos_embed(feature_trans_input4)  # (H x W) x C
        feature_rec_out4, _ = self.transformer_4(
            feature_trans_input4, pos_embed_4
        )  # (H x W) x B x C
        feature_rec_out4 = self.output_proj(feature_rec_out4)  # (H x W) x B x C
        feature_rec_4 = rearrange(
            feature_rec_out4, "(h w) b c -> b c h w", h=self.feature_size[0]
        )  # B x C X H x W
        feature_rec_4, log_jac_dets = self.nf_fast_flow2(feature_rec_4)
        feature_rec_list.append(feature_rec_4)             

        feature_rec = feature_rec_1 + 0.5*feature_rec_guide + feature_rec_3 + 0.5*feature_rec_4
        ...
        return {
            ...
            "feature_align_level": feature_align_level,
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        return self.forward_post(src, src_mask, src_key_padding_mask, pos) * 0.5 + self.forward_pre(src, src_mask, src_key_padding_mask, pos) * 0.5
