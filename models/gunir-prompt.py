...
class GUniR(nn.Module):
    ...
    def forward(self, input):
        features_norm_images = input["features_norm_images"]  # xguide, xgc, xg'
        text_descriptions = ["X$_{guide}$ ", "X$_{gc}$", "X$_{g}$' ", "F$_{in1}$", "F$_{in2}$", "F$_{d1}$", "F$_{d2}$", "F$_{rec1}$", "F$_{rec2}$", "F$_{rec}$"]
            
        feature_align = input["feature_align"]  # B x C X H x W
        
        feature_in1_single = feature_align[0] #one picture feature (C H W)
        feature_in1_single = feature_in1_single.cpu()
        feature_in1_reshape = feature_in1_single.view(272, -1).T.numpy() #(Channel)
        pca = PCA(n_components = 2)
        features_in1_reduced = pca.fit_transform(feature_in1_reshape)
        features_in1_norm = np.linalg.norm(features_in1_reduced, axis=1)
        features_in1_norm_image = features_in1_norm.reshape(14, 14)
        features_norm_images.append(features_in1_norm_image)

        #feature_align_4plus, log_jac_dets = self.nf_fast_flow(feature_align)

        feature_tokens = rearrange(
            feature_align, "b c h w -> (h w) b c"
        )  # (H x W) x B x C
        feature_rec_list = []
        feature_resize_level_list = input["feature_resize_level_list"]  # B x C X H x W 3
        ...
        for i in range(2):
            ...
            if i == 0:
                feature_d1_single = rearrange(
                    output_decoder, "(h w) b c -> b c h w", h=self.feature_size[0]
                )[0]
            if i == 1:
                feature_d2_single = rearrange(
                    output_decoder, "(h w) b c -> b c h w", h=self.feature_size[0]
                )[0] 
            ...
            if i < 3 :
                feature_level_tokens = rearrange(
                    feature_resize_level_list[i], "b c h w -> (h w) b c"
                )  # (H x W) x B x C
                feature_tokens = feature_rec_tokens + feature_level_tokens  # (H x W) x B x C
                feature_tokens_fastflow = rearrange(
                    feature_tokens, "(h w) b c -> b c h w", h=self.feature_size[0]
                )  # B x C X H x W
                
                if i == 0 :
                    feature_in2_single = feature_tokens_fastflow[0]
                
                feature_tokens_fastflow, log_jac_dets = self.nf_fast_flow(feature_tokens_fastflow)
                feature_tokens = rearrange(
                    feature_tokens_fastflow, "b c h w -> (h w) b c"
                )  # (H x W) x B x C
            ...
            feature_rec, log_jac_dets = self.nf_fast_flow(feature_rec)
            
            if i == 0:
                feature_rec1_single = feature_rec[0]
            if i == 1:
                feature_rec2_single = feature_rec[0]    
            
            feature_rec_list.append(feature_rec)

        for a in range(len(feature_rec_list)):
            if a < (len(feature_rec_list)-1):
                feature_rec = feature_rec + feature_rec_list[a]
        feature_rec = feature_rec / len(feature_rec_list)
        
        feature_rec_single = feature_rec[0]
        
        feature_in2_single = feature_in2_single.cpu()
        feature_in2_reshape = feature_in2_single.view(272, -1).T.numpy() #(Channel)
        features_in2_reduced = pca.fit_transform(feature_in2_reshape)
        features_in2_norm = np.linalg.norm(features_in2_reduced, axis=1)
        features_in2_norm_image = features_in2_norm.reshape(14, 14)
        features_norm_images.append(features_in2_norm_image)
        
        feature_d1_single = feature_d1_single.cpu()
        feature_d1_reshape = feature_d1_single.view(256, -1).T.numpy() #(Channel)
        features_d1_reduced = pca.fit_transform(feature_d1_reshape)
        features_d1_norm = np.linalg.norm(features_d1_reduced, axis=1)
        features_d1_norm_image = features_d1_norm.reshape(14, 14)
        features_norm_images.append(features_d1_norm_image)
        
        feature_d2_single = feature_d2_single.cpu()
        feature_d2_reshape = feature_d2_single.view(256, -1).T.numpy() #(Channel)
        features_d2_reduced = pca.fit_transform(feature_d2_reshape)
        features_d2_norm = np.linalg.norm(features_d2_reduced, axis=1)
        features_d2_norm_image = features_d2_norm.reshape(14, 14)
        features_norm_images.append(features_d2_norm_image)
        
        feature_rec1_single = feature_rec1_single.cpu()
        feature_rec1_reshape = feature_rec1_single.view(272, -1).T.numpy() #(Channel)
        features_rec1_reduced = pca.fit_transform(feature_rec1_reshape)
        features_rec1_norm = np.linalg.norm(features_rec1_reduced, axis=1)
        features_rec1_norm_image = features_rec1_norm.reshape(14, 14)
        features_norm_images.append(features_rec1_norm_image)
        
        feature_rec2_single = feature_rec2_single.cpu()
        feature_rec2_reshape = feature_rec2_single.view(272, -1).T.numpy() #(Channel)
        features_rec2_reduced = pca.fit_transform(feature_rec2_reshape)
        features_rec2_norm = np.linalg.norm(features_rec2_reduced, axis=1)
        features_rec2_norm_image = features_rec2_norm.reshape(14, 14)
        features_norm_images.append(features_rec2_norm_image)
        
        feature_rec_single = feature_rec_single.cpu()
        feature_rec_reshape = feature_rec_single.view(272, -1).T.numpy() #(Channel)
        features_rec_reduced = pca.fit_transform(feature_rec_reshape)
        features_rec_norm = np.linalg.norm(features_rec_reduced, axis=1)
        features_rec_norm_image = features_rec_norm.reshape(14, 14)
        features_norm_images.append(features_rec_norm_image)
        
        #plt.figure(figsize=(150,2.0))
        #for i in range(10):
        #    plt.subplot(1,10,i+1)
        #    plt.imshow(features_norm_images[i], cmap='viridis')
        #    plt.colorbar()
        #    plt.title("Feature Map {}".format(i+1), fontsize=10)
        #    plt.text(0.5, -0.15, text_descriptions[i], ha='center', va='top', transform=plt.gca().transAxes, fontsize=10)
        #plt.subplots_adjust(left=0.01,right=0.985,top=0.98,bottom=0.01,wspace=0.35,hspace=0)
        #plt.show()
        ...
        return {
            "feature_rec_list": feature_rec_list,
