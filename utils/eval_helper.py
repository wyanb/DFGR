...
def dump(save_dir, outputs):
    ...
    feature_aligns_level = outputs["feature_align_level"].cpu().numpy()  # B x C x H x W    
    feature_aligns = outputs["feature_align"].cpu().numpy()  # B x C x H x W
    feature_lists = outputs["feature_list"].cpu().numpy()  # B x C x H x W
    ...
    for i in range(batch_size):
        ...
        np.savez(
            ...
            pred=preds[i],
            mask=masks[i],  #1hw
            feature_align_level=feature_aligns_level[i], #chw
            feature_align=feature_aligns[i],  # chw
            feature_list=feature_lists[i],  # chw
            height=heights[i],
            width=widths[i],
            clsname=clsnames[i],
        )
...
def merge_together(save_dir):
    ...
    for npz_file in npz_file_list:
        ...
        masks.append(npz["mask"])   # n x 1hwa
        feature_aligns_level.append(npz["feature_align_level"])   # n x chw
        feature_aligns.append(npz["feature_align"])  # n x chw
        feature_lists.append(npz["feature_list"])  # n x chw
    preds = np.concatenate(np.asarray(preds), axis=0)  # N x H x W
    masks = np.concatenate(np.asarray(masks), axis=0)  # N x H x W
    feature_aligns_level = np.concatenate(np.asarray(feature_aligns_level), axis=0)  # N x H x W
    feature_aligns = np.concatenate(np.asarray(feature_aligns), axis=0)  # N x H x W
    feature_lists = np.concatenate(np.asarray(feature_lists), axis=0)  # N x H x W
    return fileinfos, preds, masks, feature_aligns,feature_aligns_level,feature_lists
