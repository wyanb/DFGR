...
class EvalImageStd(EvalImage):
    ...
    def encode_pred(preds):
        N, _, _ = preds.shape
        #print('N', N)
        #print('N reshape(N, -1)', preds.reshape(N, -1))
...
class EvalImageMax(EvalImage):
    ...
    def encode_pred(preds, avgpool_size):
        ...
        #print('max preds[:, None, ...]', type(preds))
        #print('max preds[:, None, ...]', preds.size())
        preds = (
            F.avg_pool2d(preds, avgpool_size, stride=1).cpu().numpy()
        )  # N x 1 x H x W
        #print('max preds0', type(preds))
        #print('max preds shape', preds.shape)
